...
<?php
 ...
 $dbh = pg_connect("host=localhost dbname=$base user=openpg password=openpgpwd");
     ...
     // execute query
     $sql = "SELECT * FROM res_partner where name='".$_POST[name]."'";
 ...
 if($total) {
    echo '<table data-role="table" id="movie-table" data-mode="reflow" class="ui-responsive table-stroke">'."\n";
			...
			echo '<th data-priority="1">active</th>';
			echo '<th data-priority="2">supplier</th>';
			echo '<th data-priority="2">customer</th>';
            echo '</tr>'."\n";
			echo '</thead>';
			echo '<tbody>';
  while ($row = pg_fetch_array($result)) {
                ...
                echo '<td bgcolor="#dddddd">'.$row["supplier"].'</td>';
				echo '<td bgcolor="#dddddd">'.$row["customer"].'</td>';
 ...
 <div data-role="header" data-theme="b">
<a title="Acceile" href="recherche.php" rel="external" data-icon="plus" data-iconpos="notext" data-direction="reverse" class="ui-btn-right-bis jqm-home ui-btn  ui-btn-icon-notext ui-btn-corner-all"><span class="ui-icon  ui-icon-plus"></span></a>
