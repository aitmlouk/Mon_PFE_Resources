...
<a title="Liste des utilisateurs" href="affichage.php" rel="external" data-icon="grid" data-iconpos="notext" data-direction="reverse" class="ui-btn-right-bis jqm-home ui-btn  ui-btn-icon-notext ui-btn-corner-all"><span class="ui-icon  ui-icon-grid"></span></a>
	...
	<h1>Liste des commandes du ventes</h1>
...
<?php
 ...
 // execute query
 $sql = "SELECT * FROM account_voucher";
 ...
 if($total) {
    echo '<table data-role="table" id="movie-table" data-mode="reflow" class="ui-responsive table-stroke">'."\n";
			...
			echo '<th data-priority="2">reference</th>';
			echo '<th data-priority="1">partner id</th>';
			echo '<th data-priority="1">amount </th>';
  ...
  while ($row = pg_fetch_array($result)) {
				...
				echo '<td bgcolor="#dddddd">'.$row["reference"].'</td>';
				echo '<td bgcolor="#dddddd">'.$row["partner_id"].'</td>';
				echo '<td bgcolor="#dddddd">'.$row["amount"].'</td>';
                echo '<td bgcolor="#dddddd">'.$row["state"].'</td>';
				echo '<td bgcolor="#dddddd">'.$row["date"].'</td>';
                echo '</tr>'."\n";}
				echo '</tbody>';
				echo '</table>'."\n";
            }
...
<div data-role="footer">		
	<div data-role="navbar">
		<ul>
			...
			<li><a href="achat.php">achat Confirmer</a></li>
			<li><a href="progress.php">facture</a></li>
		</ul>
	</div>
<div data-role="header" data-theme="b">
