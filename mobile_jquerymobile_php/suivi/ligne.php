...
<?php
 ...
 // execute query
 $sql = "SELECT * FROM account_invoice_line where invoice_id='".$_GET["id"]."'";
 ...
 if($total) {
    echo '<table data-role="table" id="movie-table" data-mode="reflow" class="ui-responsive table-stroke">'."\n";
			...
			echo '<th data-priority="2">price subtotal</th>';
            echo '</tr>'."\n";
			echo '</thead>';
			echo '<tbody>';
  while ($row = pg_fetch_array($result)) {
				...
				echo '<td bgcolor="#dddddd">'.$row["quantity"].'</td>';
				echo '<td bgcolor="#dddddd">'.$row["price_unit"].'</td>';
                echo '<td bgcolor="#dddddd">'.$row["price_subtotal"].'</td>';
